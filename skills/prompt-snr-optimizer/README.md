...
1. **Auto-Typology:** Classifies the payload as `System`, `Task`, or `Knowledge`.
...
4. **Logic Dehydration:** Removes politeness padding, narrative transitions, and vague language, rewriting soft language into strong execution verbs.
...
7. **Injection Hardening:** Strictly boundaries trusted instructions from untrusted data.
...
Returns exactly one Markdown code block followed by a telemetry line. No conversational filler is included.
````markdown
<!-- snr-max-start -->
...
### For `/analyze`
Returns exactly one Markdown code block.
````markdown
...
This tool originated as a personal project. Although every effort has been made during its development to ensure it is a highly functional, safe, and reliable tool, it is provided **"as is"**, completely without warranty of any kind, express or implied. 

Use of this tool is strictly at your own risk. The author assumes no liability or responsibility for any lost data, misconfigured prompts, unintended AI behaviors, or any direct or indirect damages arising from the use of this software. 
